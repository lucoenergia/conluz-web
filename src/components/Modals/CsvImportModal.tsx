...
export interface CsvImportResult {
  createdCount: number;
  /** `item` is the key the row is reported under; null renders the message alone. */
  errors: { item: string | null; message?: string }[];
}

export interface CsvImportHandlers {
...
export interface CsvImportModalProps {
  ...
  /** What the success line counts, as "Se ha(n) creado N <noun>". */
  createdNoun: { one: string; other: string };
  importFile: (file: File, communityId: string, handlers: CsvImportHandlers) => void;
}
...
export const CsvImportModal: FC<CsvImportModalProps> = ({
  ...
  const handleImport = () => {
    ...
    importFile(file, activeCommunityId, {
      onSuccess: (data) => {
        setResult(data);
        setStep("results");
        onImportComplete?.();
        ...
        <>
          <Box sx={{ textAlign: "center", mb: 3 }}>
            ...
            >
              ...
              {createdCount !== 1 ? createdNoun.other : createdNoun.one}
            </Typography>
          </Box>
