...
- **What each baseline captures (capture the subject, not the page):**
  - **Component subject** (dialog, menu, drawer, panel, section, bar, header): capture that component's locator, chosen by the selector hierarchy above, e.g. `expect(page.getByRole("menu")).toHaveScreenshot(...)` or `expect(page.getByTestId("modal-panel")).toHaveScreenshot(...)`.
  - **Page-layout subject**: `await expect(page).toHaveScreenshot("x.png", await mainRegion(page))` (`tests/visual/fixtures/capture.ts`). It captures `main` found by role, masks the fixed app bar (`banner`) that overlaps its top, and clips a full-page capture rather than taking an element screenshot, because the scroll an element screenshot does collapses the mobile detail header and never stabilises.
  - **Full page** only with a one-line reason at the call: `/login` (no `main`, no app bar) and the chrome canaries.
  - **The chrome (app bar, side menu) belongs to `tests/visual/chrome-canary.spec.ts` alone.** A header or menu change should fail the canaries, not every page baseline.
  - Placeholder data rendered from `Math.random()` (the kWh figures on supply cards) is masked by text in the spec that shows it. Never rely on the tolerance to absorb it.
- **Adding a screenshot name: run with `--update-snapshots=none`.** Playwright's default (`missing`) **writes** a baseline for any name that has none, which is exactly the baseline generation agents must not do. Report the new name as missing and let the maintainer generate it.
