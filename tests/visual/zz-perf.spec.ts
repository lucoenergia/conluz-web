...
const LATENCY = 300; // simulate a real round-trip; fixtures otherwise resolve instantly

test.describe("integrations load", () => {
  test("measure", async ({ page }) => {
    const log: { u: string; t: number }[] = [];
    const t0 = Date.now();
    const delayed = (body: unknown) => async (route: Route) => {
      log.push({ u: new URL(route.request().url()).pathname, t: Date.now() - t0 });
      await new Promise((r) => setTimeout(r, LATENCY));
      await route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify(body) });
    };

    await injectAuthToken(page);
    await seedActiveCommunity(page, FIXED_PLATFORM_ADMIN_USER.id);
    await mockAllApiRoutes(page, FIXED_PLATFORM_ADMIN_USER);
    await page.route((u) => u.pathname.endsWith("/config/shelly"), delayed({ enabled: true }));
    await page.route((u) => u.pathname.endsWith("/config/datadis"), delayed({ enabled: true, username: "u", baseUrl: "https://datadis.es/api-private" }));
    await page.route((u) => /\/plants$/.test(u.pathname), delayed({ items: [{ id: "plant-1", name: "Planta" }], total: 1 }));
    await page.route((u) => u.pathname.includes("/production/huawei/config"), delayed({ enabled: true, username: "h", baseUrl: "https://eu5.fusionsolar.huawei.com/thirdData" }));

    const start = Date.now();
    await page.goto("/integrations");

    const at = async (sel: string) => {
      try { await page.waitForSelector(sel, { timeout: 8000 }); return Date.now() - start; } catch { return -1; }
    };
    const chrome = await at("nav[aria-label='breadcrumbs']");
    const cards = await at("text=Datadis");

    console.log("URL:", page.url());
    console.log("REQUESTS:", JSON.stringify(log.map((r) => `${r.t}ms ${r.u.replace(/^\/api\/v1/, "")}`)));
    console.log("TIMING:", JSON.stringify({ chromeMs: chrome, cardsMs: cards, latency: LATENCY }));
  });
});
